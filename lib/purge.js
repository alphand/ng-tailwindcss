...
module.exports = ({ keyframes, fontFace, rejected, outputFile }) => {
  ...
  if (fs.existsSync(ngTwFile)) {
    ...
    const purgecss = new Purgecss({
      ...
      css: [path.normalize(path.resolve(process.cwd(), outputFile))],
      extractors: [
        {
          extractor: class {
    ...
    fs.writeFile(path.resolve(process.cwd(), outputFile), newCSS, err =>
      err ? console.error(err) : console.info('CSS Purged')
    );
  } else {
